...
<head>
    <meta charset="UTF-8">
    <title>IPCR â€“ For Review / Calibration Reference</title>
    <style>
        ...
        * { -webkit-print-color-adjust: exact; }
        ...
        .indicator-row { padding: 4px 0; font-size: 11px; text-align: left; }
        .indicator-row.border-b { border-bottom: 1px solid #000; }
    </style>
</head>
<body>
    <div class="header">
        <h3 style="margin:2px 0;">INDIVIDUAL PERFORMANCE COMMITMENT AND REVIEW (IPCR)</h3>
        <div class="small" style="margin-top:2px;">IPCR â€“ For Review / Calibration Reference</div>
    </div>
    ...
    <table class="no-border" style="margin-bottom: 4px;">
        ...
        <tr>
            <td style="text-align:left;">Employee: <u>{{ $employeeName }}</u> &nbsp;&nbsp; Office/Division: <u>{{ $officeDivision }}</u> &nbsp;&nbsp; Position: <u>{{ $position }}</u></td>
    ...
    <table>
        <thead>
            <tr>
                <th rowspan="2" style="width:16%;">OUTPUT</th>
                <th rowspan="2" style="width:28%;">Success Indicators (Measure + Target)</th>
                <th rowspan="2" style="width:10%;">6 Months Summary of Accomplishment</th>
                <th colspan="4" style="width:12%;">Rating</th>
                <th rowspan="2" style="width:12%;">Remarks</th>
                <th colspan="5" style="width:22%;">Standards per Success Indicator</th>
            </tr>
            <tr class="small grey">
        ...
        <tbody>
            <tr>
                <td class="section-label" colspan="13">A. CORE FUNCTIONS (80%)</td>
            </tr>
                ...
                <tr>
                    ...
                    <td class="left">
                        @foreach ($item['success_indicators'] as $indicator)
                            <div class="indicator-row {{ !$loop->last ? 'border-b' : '' }}">{{ $indicator }}</div>
                        @endforeach
                    </td>
                    <td>{{ $item['accomplishment'] }}</td>
                    <td>{{ $item['q'] }}</td><td>{{ $item['e'] }}</td><td>{{ $item['t'] }}</td><td>{{ $item['a'] }}</td>
                    <td class="remarks">{{ $item['remarks'] }}</td>
                    <td>{{ $item['standard'] }}</td><td></td><td></td><td></td><td></td>
                </tr>
            @endforeach

            <tr>
                <td class="section-label" colspan="13">C. SUPPORT FUNCTIONS (20%)</td>
            </tr>
    ...
    <table class="no-border" style="margin-top:6px;">
        <tr>
            ...
            <td style="border:1px solid #000; width:12%;">{{ $weightedCore }}</td>
        </tr>
        <tr>
            <td class="left">Weighted Average Rating for Support Functions (20%)</td>
            <td style="border:1px solid #000;">{{ $weightedSupport }}</td>
        </tr>
        <tr>
            <td class="left"><strong>OVERALL RATING</strong></td>
            <td style="border:1px solid #000;">{{ $overallRating }}</td>
        </tr>
        <tr>
            <td class="left"><strong>ADJECTIVAL RATING</strong></td>
            <td style="border:1px solid #000;">{{ $adjectival }}</td>
        </tr>
    </table>
    ...
    <div class="small" style="margin-top:10px;">
        Derived from locked SMPOR and Supervisor IPCR Rating. Subject to PMT calibration.
    </div>
</body>
</html>
