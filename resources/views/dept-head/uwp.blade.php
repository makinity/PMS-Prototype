        ...
        <div class="mt-4 space-y-4">
            ...
            <div class="overflow-hidden rounded-xl border border-slate-800 bg-slate-900/40">
                <table class="w-full text-sm text-slate-100">
                    ...
                    <tbody id="uwp-assignees-table-body" class="divide-y divide-slate-800">
                        {{-- Rendered by JS --}}
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    // NOTE: supports multiple assignees per indicator (badge count), but flow says 1 per indicator.
            const uwpAssigneesByMfo = {
                "E-Bank Scanning and Encoding of Revenue Transactions": {
    ...
    function getAssignedEmployeesForIndicator(mfoTitle, indicator) {
        const mfoMap = uwpAssigneesByMfo[mfoTitle] || {};
        const assigned = mfoMap[indicator] || [];
    ...
    function openStandardsViewer(mfoTitle, indicator) {
        ...
        if (label) label.textContent = indicator;
    ...
    function renderAssigneesTable(unit, indicator, assignees) {
        ...
        list.forEach((name) => {
            ...
            tr.className = 'hover:bg-slate-900/30';
            tr.innerHTML = `
                <td class="px-4 py-3 text-slate-100">${name}</td>
                <td class="px-4 py-3 text-slate-300">${unit}</td>
                <td class="px-4 py-3 text-slate-300">${indicator}</td>
                <td class="px-4 py-3">
    ...
    function openAssigneesViewer(unit, mfoTitle, indicator) {
        ...
        const assignees = (uwpAssigneesByMfo[mfoTitle] && uwpAssigneesByMfo[mfoTitle][currentIndicator])
            ? uwpAssigneesByMfo[mfoTitle][currentIndicator]
            : [];
    ...
    (function initIndicatorsModal() {
        ...
        function buildBadge(count) {
            if (!count) {
                return `<span class="inline-flex items-center rounded-full border border-slate-500/40 bg-slate-500/10 px-3 py-1 text-xs font-semibold text-slate-200">Unassigned</span>`;
            }
            return `<span class="inline-flex items-center rounded-full border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-300">Assigned (${count})</span>`;
        }

        function openIndicatorsModal(title, unit, mfoTitle, indicators) {
            titleEl.textContent = title || '--';
            tableBody.innerHTML = '';

            (indicators || []).forEach((text) => {
                const indicator = (text || '').trim();
                if (!indicator) return;

                const assignees = (uwpAssigneesByMfo[mfoTitle] && uwpAssigneesByMfo[mfoTitle][indicator])
                    ? uwpAssigneesByMfo[mfoTitle][indicator]
                    : [];
                const count = Array.isArray(assignees) ? assignees.length : 0;
                const primaryName = count === 1 ? assignees[0] : '';
            ...
            document.querySelectorAll('[data-uwp-view-indicators]').forEach((btn) => {
                ...
                openIndicatorsModal(
                    btn.dataset.title || '--',
                    btn.dataset.unit || 'Revenue Collection Unit',
                    btn.dataset.title || '',
                    indicators
                );
            });
    ...
    (function initViewUwpModal() {
        const run = () => {
            ...
            if (!viewModal || !viewTriggers.length) return;
            ...
            const showViewModal = () => {
                ...
                else viewModal.classList.remove('hidden');
            };

            const hideViewModal = () => {
                if (viewModalInstance) viewModalInstance.hide();
                else viewModal.classList.add('hidden');
            };
