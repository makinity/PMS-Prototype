...
@section('main-content')
    <section class="space-y-5">
        ...
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-400">Supervisor Review</p>
            ...
            <a href="{{ route('supervisor.employee-submissions') }}"
            ...
            <div class="rounded-xl border border-gray-700 bg-slate-900/40 p-4">
                ...
                @php
                    $badgeClass = match ($status) {
                        'submitted_to_supervisor' => 'border-blue-500/30 bg-blue-500/10 text-blue-200',
                        'supervisor_endorsed' => 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200',
                        'dept_head_endorsed' => 'border-violet-500/30 bg-violet-500/10 text-violet-200',
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div class="rounded-xl border border-gray-700 bg-slate-900/40 p-4">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <a href="{{ route('supervisor.submissions.ipcr-preview', $submission->id) }}"
                        class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 text-slate-200 transition hover:bg-slate-800">
                        <i class="fa-regular fa-eye text-sm"></i>
                    </a>
                </div>
            ...
            @if ($status === 'submitted_to_supervisor')
                <div class="flex items-center justify-end">
                    <button type="button" id="btnEndorseSubmission"
                        ...
                        <span id="endorse-label">Endorse to Department Head</span>
                    </button>
                </div>
            @elseif ($status === 'supervisor_endorsed')
                <div class="text-center text-sm text-emerald-300">
                    <i class="fa-solid fa-check-circle mr-1"></i> Endorsed to Department Head.
                </div>
            @else
    ...
    <script>
    (function() {
        const btn = document.getElementById('btnEndorseSubmission');
        if (!btn) return;
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('{{ route("supervisor.submissions.endorse", $submission->id) }}', {
                    method: 'POST',
                    headers: {
                ...
                if (badge) {
                    badge.textContent = 'Supervisor Endorsed';
                ...
                document.getElementById('action-section').innerHTML =
                    '<div class="text-center text-sm text-emerald-300"><i class="fa-solid fa-check-circle mr-1"></i> Endorsed to Department Head.</div>';

                // Snackbar
                showSnackbar('Endorsed to Department Head successfully.');
            } catch (err) {
                btn.disabled = false;
                if (spinner) spinner.classList.add('hidden');
                if (label) label.textContent = 'Endorse to Department Head';
