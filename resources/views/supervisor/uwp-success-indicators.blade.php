...
@section('main-content')
    ...
    <section class="space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <a href="{{ route('supervisor.uwp', ['uwp_id' => $uwp->id]) }}"
        ...
        <form method="POST" action="{{ route('supervisor.uwp.success-indicators.save', ['uwpId' => $uwp->id, 'mfoId' => $mfo->id]) }}"
              class="flex items-center justify-end gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 px-5 py-4">
            @csrf
            <input type="hidden" name="indicators_payload" id="si-indicators-payload" value="">
            <a href="{{ route('supervisor.uwp', ['uwp_id' => $uwp->id]) }}"
            ...
            @if($canEdit)
                <button type="submit"
                        class="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-cyan-900/30 transition hover:bg-cyan-500">
                    Save &amp; Return to UWP Editor
                </button>
            @endif
        </form>
    </section>
            ...
            (function () {
                ...
                const siState = {
                    indicators: Array.isArray(initialIndicators) ? initialIndicators.map((i) => ({
                        ...
                        targetTimeline: String(i.targetTimeline ?? ''),
                ...
                function renderAssignees(indicator) {
                    ...
                    if (!assignedData.length) {
                        assigneesListEl.innerHTML = '<tr><td colspan="3" class="px-4 py-6 text-center text-xs text-slate-500">No employees found.</td></tr>';
                        return;
                    }

                    const assigned = new Set((indicator.assignees || []).map((x) => Number(x)));
                    assignedData.forEach((emp) => {
                        const isAssigned = assigned.has(Number(emp.id));
                    ...
                    if (canEdit) {
                        assigneesListEl.querySelectorAll('[data-toggle-assignee]').forEach((btn) => {
                            btn.addEventListener('click', () => {
                                const empId = Number(btn.getAttribute('data-toggle-assignee'));
                                if (!empId) return;
                                indicator.assignees = Array.isArray(indicator.assignees) ? indicator.assignees : [];
                                const idx = indicator.assignees.indexOf(empId);
                                if (idx === -1) indicator.assignees.push(empId);
                                else indicator.assignees.splice(idx, 1);
                                renderDetail(indicator);
                            });
                        });
                    }
