...
@section('main-content')
    ...
    <section class="space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-white">Employee Submissions</h1>
                <p class="text-sm text-slate-400">Review submitted SMPOR and IPCR accomplishment snapshots from your assigned employees.</p>
    ...
    <script id="supervisor-submissions-json" type="application/json">{!! json_encode($submissionPayloads ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
    ...
    <div id="submission-view-modal" data-preview-modal class="fixed inset-0 z-[80] hidden items-center justify-center bg-black/60 px-4 py-6">
        <div class="w-full max-w-5xl rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl">
            <div class="flex items-start justify-between gap-3 border-b border-slate-800 pb-3">
                <div>
                    <p class="text-xs uppercase tracking-[0.2em] text-sky-300">Supervisor Review</p>
                    <h3 class="text-lg font-semibold text-white">Employee Submission Details</h3>
    ...
    <div id="supervisor-smpor-preview-modal" data-preview-modal data-parent-modal-id="submission-view-modal" class="fixed inset-0 z-[90] hidden items-center justify-center bg-black/60 px-4 py-6">
    ...
    <div id="supervisor-ipcr-preview-modal" data-preview-modal data-parent-modal-id="submission-view-modal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/60 px-4 py-6">
    ...
    <div id="supervisor-ipcr-indicators-modal" data-preview-modal data-parent-modal-id="supervisor-ipcr-preview-modal" class="fixed inset-0 z-[110] hidden items-center justify-center bg-black/60 px-4 py-6">
    ...
    <div id="supervisor-ipcr-standards-modal" data-preview-modal data-parent-modal-id="supervisor-ipcr-indicators-modal" class="fixed inset-0 z-[120] hidden items-center justify-center bg-black/60 px-4 py-6">
        <div class="w-full max-w-5xl rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl">
            <div class="flex items-start justify-between gap-3 border-b border-slate-800 pb-3">
                <div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const payloadScript = document.getElementById('supervisor-submissions-json');
                ...
                function renderSmporPreview(payload) {
                    ...
                    openPreviewModal('supervisor-smpor-preview-modal');
                }
                ...
                function renderIpcrPreview(payload) {
                    ...
                    if (!ipcrSectionsContainerEl) {
                        openPreviewModal('supervisor-ipcr-preview-modal');
                        return;
                    }
                ...
                function renderIndicatorsModal(sectionIndex, rowIndex) {
                            ...
                            : selectedIndicators.map((indicator, indicatorIndex) => {
                                ...
                                return `<tr class="bg-slate-900/40 align-top">
                                    <td class="px-4 py-3 text-slate-100">
                                        ${escapeHtml(indicator?.indicator_text || '--')}
                                    </td>
                                    <td class="px-4 py-3 text-center tabular-nums text-slate-200 whitespace-nowrap">
                                        ${qVal}
                                    </td>
                                    <td class="px-4 py-3 text-center tabular-nums text-slate-200 whitespace-nowrap">
                                        ${eVal}
                    ...
                    openPreviewModal('supervisor-ipcr-indicators-modal');
                }
                ...
                function renderStandardsModal(indicatorIndex) {
                    ...
                    openPreviewModal('supervisor-ipcr-standards-modal');
                }
