...
@section('main-content')
            ...
            <div class="border-b border-gray-700 px-5 py-4">
                <div class="flex flex-wrap items-end gap-3">
                    <div class="min-w-[220px] flex-1">
                        <label for="pmt-accomplishment-search" class="mb-1 block text-xs uppercase tracking-[0.14em] text-slate-400">Search</label>
                        <input
                    ...
                    <div class="w-full min-w-[180px] sm:w-auto">
                        <label for="pmt-accomplishment-status" class="mb-1 block text-xs uppercase tracking-[0.14em] text-slate-400">Status</label>
                        <select id="pmt-accomplishment-status"
                                data-live-submission-status
                                style="background-color:#020617;color:#e2e8f0;border-color:#334155;"
                            ...
                            <option value="supervisor_endorsed">Supervisor Endorsed</option>
                            <option value="dept_head_endorsed">Dept Head Endorsed</option>
                            <option value="recommended_by_pmt">Recommended by PMT</option>
                            <option value="approved_by_pmt">Approved by PMT</option>
                            <option value="adjusted_by_pmt">Adjusted by PMT</option>
                            <option value="released_by_pmt">Released by PMT</option>
                            <option value="returned_to_employee">Returned</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full max-w-5xl rounded-2xl border border-gray-700 bg-slate-900 p-5 shadow-2xl">
            ...
            <div class="mt-4 max-h-[68vh] space-y-4 overflow-y-auto pr-1 text-sm text-slate-200">
                ...
                <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-gray-700 bg-slate-950/60 p-4">
                ...
                <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-gray-700 bg-slate-950/60 p-4">
                        <p class="text-[11px] uppercase tracking-[0.14em] text-slate-500">Dept Head Action</p>
                        <p id="viewSubmissionDeptHeadActionAt" class="mt-2 text-sm font-semibold text-slate-100">--</p>
                        <p id="viewSubmissionDeptHeadRemarks" class="mt-2 whitespace-pre-line text-sm text-slate-300">--</p>
                    </div>
                </div>
            ...
            <div class="mt-5 flex items-center justify-end gap-4 border-t border-gray-700 pt-4">
                ...
                <form method="POST"
                    ...
                    action="{{ route('pmt.acc-review.approve', ['id' => 0]) }}"
                    class="inline">
                    @csrf
                    <button id="pmtSubmissionApproveBtn" type="submit"
        ...
        <div class="w-full max-w-6xl rounded-2xl border border-gray-700 bg-slate-900 p-5 shadow-2xl">
            ...
            <div class="mt-4 max-h-[66vh] space-y-5 overflow-y-auto pr-1 text-sm text-slate-200">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-4">
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Employee</p><p id="smporEmployeeName" class="mt-1 font-semibold">--</p></div>
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Office/Unit</p><p id="smporOfficeName" class="mt-1 font-semibold">--</p></div>
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Period</p><p id="smporPeriodLabel" class="mt-1 font-semibold">--</p></div>
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Source</p><p id="smporSourceLabel" class="mt-1 font-semibold">--</p></div>
                </div>
        ...
        <div class="w-full max-w-7xl rounded-2xl border border-gray-700 bg-slate-900 p-5 shadow-2xl">
            ...
            <div class="mt-4 max-h-[66vh] space-y-4 overflow-y-auto pr-1 text-sm text-slate-200">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Employee</p><p id="ipcrEmployeeName" class="mt-1 font-semibold">--</p></div>
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Office/Unit</p><p id="ipcrOfficeName" class="mt-1 font-semibold">--</p></div>
                    <div class="rounded-lg border border-gray-700 bg-slate-950/60 p-3"><p class="text-[11px] uppercase text-slate-500">Period</p><p id="ipcrPeriodLabel" class="mt-1 font-semibold">--</p></div>
                </div>
    ...
    <div id="pmt-ipcr-indicators-modal" data-preview-modal data-parent-modal-id="pmt-ipcr-preview-modal" class="fixed inset-0 z-[110] hidden items-center justify-center bg-black/60 px-4 py-6">
    ...
    <div id="pmt-ipcr-standards-modal" data-preview-modal data-parent-modal-id="pmt-ipcr-indicators-modal" class="fixed inset-0 z-[120] hidden items-center justify-center bg-black/60 px-4 py-6">
        <div class="w-full max-w-5xl rounded-2xl border border-gray-700 bg-slate-900 p-5 shadow-2xl">
            <div class="flex items-start justify-between gap-3 border-b border-gray-700 pb-3">
                <div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const submissionDeptHeadActionAtEl = document.getElementById('viewSubmissionDeptHeadActionAt');
                const submissionDeptHeadRemarksEl = document.getElementById('viewSubmissionDeptHeadRemarks');
                ...
                function applySubmissionLiveSearch() {
                    ...
                    const statusFilter = String(document.querySelector('[data-live-submission-status]')?.value || '').toLowerCase();
                    let visibleCount = 0;
                ...
                function renderSubmissionModal(payload) {
                    ...
                    if (submissionDeptHeadActionAtEl) {
                        submissionDeptHeadActionAtEl.textContent = payload.dept_head_action_at_label || '--';
                    }
                    if (submissionDeptHeadRemarksEl) {
                        submissionDeptHeadRemarksEl.textContent = (payload.dept_head_remarks || '').trim() !== '' ? payload.dept_head_remarks : '--';
                    }
                    ...
                    if (submissionApproveFormEl) {
                        const submissionId = String(payload.id ?? '').trim();
                        const actionTemplate = String(submissionApproveFormEl.dataset.actionTemplate || '');
                        if (submissionId !== '' && actionTemplate.includes('__SUBMISSION_ID__')) {
                            submissionApproveFormEl.action = actionTemplate.replace('__SUBMISSION_ID__', encodeURIComponent(submissionId));
                        }
                    }

                    if (submissionApproveBtnEl) {
                        const canApprove = String(payload.status || '').toLowerCase() === 'dept_head_endorsed';
                        submissionApproveBtnEl.disabled = !canApprove;
                        submissionApproveBtnEl.classList.toggle('opacity-50', !canApprove);
                        submissionApproveBtnEl.classList.toggle('cursor-not-allowed', !canApprove);
                    }
                ...
                function renderIpcrPreview(payload) {
                    ...
                    if (currentIpcrSections.length === 0) {
                        ipcrSectionsContainerEl.innerHTML = '<div class="rounded-xl border border-gray-700 bg-slate-950/60 px-4 py-8 text-center text-slate-400">No IPCR commitments found for this submission.</div>';
                    } else {
                ...
                function renderStandardsModal(indicatorIndex) {
                    ...
                    openPreviewModal('pmt-ipcr-standards-modal');
                }
                ...
                document.querySelector('[data-live-submission-status]')?.addEventListener('change', applySubmissionLiveSearch);
